...
function validateClasses(classes) {
    ...
    for (const c of classes) {
        const src = c._src || "???";
        if (!c.key)
            errors.push(`Classe sans key (fichier ${src})`);
        if (!c.label)
            errors.push(`Classe ${c.key} sans label (${src})`);
        if (!c.description)
...
function writeOutput(classes) {
    ...
    clean.sort((a, b) => (a.label || a.key).localeCompare(b.label || b.key, "fr"));
