 ...
 */
export function describeStyle(style: MQImageStyle): string {
  switch (style) {
 ...
 */
export function describeVariant(variant: MQImageVariant): string {
  switch (variant) {
    case "front":
...
// Options génériques pour construire un prompt MQ
 ...
 */
export function buildMQPrompt(options: MQBuildPromptOptions): MQGeneratedPrompt {
  const {
    key,
    label,
    style,
  ...
  const variant: MQImageVariant = options.variant ?? "front";

  const mainBase =
    (config && config.basePrompt) ||
    basePrompt ||
    `fantasy object called "${label ?? key}", detailed design, clear silhouette`;

  const parts: string[] = [mainBase];
