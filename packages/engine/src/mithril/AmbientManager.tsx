...
/**
 * On ne se bat pas avec les types ici : on relâche un peu TS pour
 * éviter de casser le reste de l’engine.
 */
type AnyAmbientConfig = {
  ambientLight?: {
    morning: string;
    day: string;
    evening: string;
  ...
  sources?: any[];
  // certaines versions de la config ne déclarent pas cette clé
  enableSpatialAudio?: boolean;
};

type AmbientSourceNode = {
  id: string;
...
type AmbientNodesMap = Map<string, AmbientSourceNode>;

const AmbientManager = () => {
  // refs principaux
  const ctxRef = useRef<AudioContext | null>(null);
  const nodesRef = useRef<AmbientNodesMap>(new Map());
  const resumeRequestedRef = useRef(false);

// caméra (position + orientation) — on relâche le typage
const cam = useCamera() as any;
const position = cam.position ?? { x: 0, y: 0, z: 0 };
const forward = cam.forward ?? { x: 0, y: 0, z: -1 };

  // on “assouplit” la config pour éviter les erreurs de type
  const AmbientConfig = AmbientConfigDefault as AnyAmbientConfig;
  const sources = (AmbientConfig.sources ?? []) as any[];
  ...
  const updateAmbience = async () => {
    ...
    // saison
    if (season === "winter") snowVol = 0.25;
    if (season === "spring") windVol = 0.2;
    if (season === "summer") windVol = 0.3;
    if (season === "autumn") windVol = 0.25;
    ...
    try {
      if (windNode) {
        windNode.gain.gain.setTargetAtTime(
          windVol,
      ...
      if (snowNode) {
        snowNode.gain.gain.setTargetAtTime(
          snowVol,
          currentCtx.currentTime,
          2.5
        );
