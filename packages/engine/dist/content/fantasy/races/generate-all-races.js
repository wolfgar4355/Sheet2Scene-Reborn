...
function validateRaces(races) {
    ...
    for (const r of races) {
        const from = r._sourceFile || "inconnu";
        if (!r.key)
            errors.push(`Race sans "key" (fichier ${from})`);
        if (!r.label)
            errors.push(`Race ${r.key || "?"} sans "label" (${from})`);
        if (!r.description)
