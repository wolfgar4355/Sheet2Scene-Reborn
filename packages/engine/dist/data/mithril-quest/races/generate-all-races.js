...
function validateRaces(races) {
    ...
    for (const r of races) {
        if (!r.key)
            errors.push(`Race sans key (${r.label})`);
        if (!r.label)
            errors.push(`Race sans label (${r.key})`);
        if (!r.description)
            errors.push(`Race sans description (${r.key})`);
        if (!Array.isArray(r.traits))
            errors.push(`Traits invalides (${r.key})`);
    }
    return errors;
}
function writeOutput(races) {
