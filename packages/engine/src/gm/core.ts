 ...
 * ----------------------------------------------------- */
function filterMonsters(all: GmeMonster[], req: EncounterRequest): GmeMonster[] {
  return all.filter((m) => {
    // --- CR ---
    ...
    // --- habitat ---
    if (req.habitat && !(m.habitatTags ?? []).includes(req.habitat)) return false;
...
export function runEncounterEngine(
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (diff <= tolerance) break; // validé
  }
