...
import { AmbientConfig, getAmbientColor, } from "../ambient.config";
...
export default function AmbientManager() {
    ...
    useEffect(() => {
        ...
        const updateListener = () => {
            ...
            if (!currentCtx || !currentCtx.listener)
                return;
            const { x, y, z } = camera;
        ...
        const updateAmbience = () => {
            ...
            try {
                if (wind) {
                    wind.gain.gain.setTargetAtTime(windVol, currentCtx.currentTime, 2.5);
                }
                if (rain) {
                    rain.gain.gain.setTargetAtTime(rainVol, currentCtx.currentTime, 2.5);
                }
                if (snow) {
                    snow.gain.gain.setTargetAtTime(snowVol, currentCtx.currentTime, 2.5);
                }
            }
            catch {
                if (wind)
                    wind.gain.gain.value = windVol;
                if (rain)
                    rain.gain.gain.value = rainVol;
                if (snow)
                    snow.gain.gain.value = snowVol;
            }
        };
        ...
        updateListener();
        // Timers
        const ambienceTimer = window.setInterval(updateAmbience, 10000);
        const tintTimer = window.setInterval(setVisualState, 5 * 60 * 1000);
