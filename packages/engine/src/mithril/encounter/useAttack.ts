...
export function useAttack() {
  ...
  const attack = useCallback(
    (
      ...
      if (d20 === 1) {
        log.push(
          `🎲 ${attacker.name} attaque ${target.name} : NAT 1 → ❌ ÉCHEC`
        );
        return;
      }
      ...
      if (!hit) {
        log.push(
          `🎲 ${attacker.name} attaque ${target.name} : ${d20}+${atkBonus}=${total} vs AC ${ac} → ❌ Miss`
        );
        return;
      }
      ...
      const dmg = rollDamage(
        dice,
        dmgSpec.sides,
        (dmgSpec.bonus ?? 0) + (opts?.bonusDamage ?? 0)
      );
      ...
      if (isCrit) {
        log.push(
          `💥 CRIT ! ${attacker.name} frappe ${target.name} → ${dmg} dégâts`
        );
      } else {
        log.push(
          `✅ ${attacker.name} touche ${target.name} → ${dmg} dégâts`
        );
      }
