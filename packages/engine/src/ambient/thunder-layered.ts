...
export interface ThunderLayer {
  ...
  delayMs: number;     // délai avant lecture
  volume: number;      // volume relatif
  detune?: number;     // variation de pitch (-200 à +200)
  stereoPan?: number;  // -1 gauche, +1 droite
}
...
export interface ThunderLayeredEvent {
  ...
  flashNow: boolean;        // true = éclair instantané
  layers: ThunderLayer[];
}
 ...
 */
function audioPair(path: string): string[] {
  return [
 ...
 */
export function generateThunderLayered(dist01: number): ThunderLayeredEvent {
  ...
  layers.push({
    ...
    delayMs: baseDelay,
    volume:
      distance === "close" ? 1.0 : distance === "mid" ? 0.8 : 0.65,
