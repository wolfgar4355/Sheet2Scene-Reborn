...
import type { FantasyClassKey } from "../character/types";
...
// ÉCOLES DE MAGIE (internes, sans accents pour éviter les emmerdes d’encodage)
...
export const FANTASY_SPELLS: Spell[] = [
  ...
  },
  {
    ...
    effect:
      "La cible doit réussir un jet approprié ou être entravée pendant quelques rounds.",
    castingTime: "action",
    duration: "rounds",
    range: "18m",
    target: "une créature",
  ...
  },
  {
    ...
    effect:
      "Crée une zone difficile et peut immobiliser temporairement une cible.",
    castingTime: "action",
    duration: "rounds",
    range: "9m",
  ...
  },
  {
    ...
    effect:
      "Accorde un avantage narratif sur certaines décisions, recherches ou négociations.",
    castingTime: "minute",
    duration: "minutes",
    range: "soi-même",
    target: "lanceur",
  ...
  // ----- NIVEAU 5 -----
  {
    ...
    effect:
      "Soigne plusieurs alliés dans une large zone.",
    castingTime: "action",
    duration: "instant",
    range: "9m",
  ...
  // ----- NIVEAU 7 -----
  {
    ...
    effect:
      "Crée une zone de ténèbres magiques, très difficile à percer même magiquement.",
    castingTime: "action",
    duration: "minutes",
    range: "27m",
    target: "zone",
  ...
  // ----- NIVEAU 8 -----
  {
    ...
    effect:
      "Réduit fortement les dégâts, confère bonus majeurs de défense pour un groupe.",
    castingTime: "minute",
    duration: "minutes",
    range: "9m",
    target: "groupe",
  ...
  // ----- NIVEAU 9 -----
  {
    ...
    effect:
      "Crée un événement magistral (tempête, explosion, rupture de réalité) avec conséquences durables.",
    castingTime: "minute",
    duration: "permanent",
    range: "ligne de vue",
...
export const BASIC_SPELLS: Spell[] = FANTASY_SPELLS.filter((s) => s.rank <= 1);

// ===========================
// PROGRESSION MAGIQUE PAR CLASSE
// ===========================
...
export interface ClassMagicProgression {
  ...
  manaPerLevel: number[]; // index = niveau, index 0 inutilisé
}

// Progression simple v1 (tu pourras l’ajuster à ta sauce)
export const FANTASY_MAGIC_PROGRESSION: ClassMagicProgression[] = [
  {
    classKey: "archmage",
    casterType: "full",
    maxRank: 9,
...
export function getSpellsForClass(
  ...
  return FANTASY_SPELLS.filter((s) => {
    if (!s.classTags.includes(classKey)) return false;
    if (typeof maxRank === "number" && s.rank > maxRank) return false;
    return true;
  });
}
...
export function getManaForLevel(
  ...
  if (!prog) return null;
  if (level < 1 || level >= prog.manaPerLevel.length) return null;
