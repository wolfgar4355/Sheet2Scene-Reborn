...
import type { ThunderLayer, ThunderLayeredEvent } from "./thunder-layered";
import { getAudioAsset } from "./manifest";

export interface MithrilAudioEngineOptions {
  useWorklet?: boolean;  // si false => fallback classique
}
...
export class MithrilAudioEngine {
  ...
  constructor(private opts: MithrilAudioEngineOptions = {}) {}
   ...
   */
  async playLoopFromUrls(urls: string[], volume = 1): Promise<GainNode | null> {
   ...
   */
  async playThunderEvent(event: ThunderLayeredEvent) {
    const { layers } = event;
 ...
 */
let _engine: MithrilAudioEngine | null = null;
