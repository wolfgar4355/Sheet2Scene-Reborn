 ...
 */
export default function WorldAmbienceController({ biome, timeOfDay = "day", intensity = 1, }) {
    useEffect(() => {
        ...
        if (biome === "forest") {
            ...
            const main = timeOfDay === "night" ? src.night : src.day;
            audioBus.playLoop("world-bed", main, clamp(0.6 * intensity));
            audioBus.playLoop("world-extra-1", src.creaking, clamp(0.3 * intensity));
        }
            ...
            audioBus.playLoop("world-extra-1", src.ruinsEcho, clamp(0.4 * intensity));
        }
        else if (biome === "arctic") {
            const src = world.arctic;
            audioBus.playLoop("world-bed", src.blizzard, clamp(0.7 * intensity));
            audioBus.playLoop("world-extra-1", src.iceCrack, clamp(0.35 * intensity));
        }
        else if (biome === "volcano") {
            const src = world.volcano;
            audioBus.playLoop("world-bed", src.lavaRumble, clamp(0.7 * intensity));
            audioBus.playLoop("world-extra-1", src.lavaBubble, clamp(0.45 * intensity));
            ...
            audioBus.playLoop("world-extra-1", src.fireplace, clamp(0.4 * intensity));
        }
        else if (biome === "ruins") {
            const src = world.ruins;
            audioBus.playLoop("world-bed", src.echo, clamp(0.6 * intensity));
            audioBus.playLoop("world-extra-1", src.stoneFall, clamp(0.35 * intensity));
            audioBus.playLoop("world-extra-2", src.whisper, clamp(0.3 * intensity));
        }
        return () => {
