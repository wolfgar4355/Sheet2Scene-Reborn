...
export interface MQEraPose {
  key: string;          // eraKey + suffix
  eraKey: string;       // ex: "age-of-legends"
  label: string;        // ex: "Vision mythique"
  camera: string;       // portrait / paysage / vista
  mood: string;         // epic / ancient / dark / futuristic…
  lighting: string;     // divine / natural / storm / cosmic…
  environment: string;  // décor
  gesture: string;      // posture du sujet
  expression: string;   // ambiance générale
  style?: string;       // clé IA optionnelle
}
...
function autoPoseForEra(eraKey: string): MQEraPose {
...
export const MQ_ERA_POSES: MQEraPose[] = ALL_MQ_ERAS.map(era =>
  autoPoseForEra(era.key)
) as const;
