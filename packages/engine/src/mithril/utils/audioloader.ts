 ...
 */
export async function loadAudio(srcBase: string): Promise<HTMLAudioElement> {
  // Déjà chargé ?
  ...
  if (supports("audio/ogg")) chosen = ogg;
  else if (supports("audio/mpeg")) chosen = mp3;

  // Fallback si aucun format n’est détecté
  if (!chosen) chosen = mp3;
