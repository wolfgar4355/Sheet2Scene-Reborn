...
function extractArrayFromText(text, varName) {
    const regex = new RegExp(`export\\s+const\\s+${varName}\\s*=\\s*\\[([\\s\\S]*?)\\]\\s*(?:as\\s+const)?\\s*;`, "m");
...
function extractArrayFromFile(filename, varName) {
    ...
    if (!arr) {
        console.warn(`⚠ ${varName} non trouvé dans ${filename} (utilisation d'un tableau vide)`);
        return [];
    }
    console.log(`🟢 ${varName} chargé depuis ${filename} (${arr.length} éléments)`);
    return arr;
}
/**********************************************************************
...
function buildPlayerDictionary() {
    const dict = {
        ...
        alignmentIcons: extractArrayFromFile("alignements-icons.ts", "MQ_ALIGNMENT_ICONS"),
        alignmentPoses: extractArrayFromFile("alignements-poses.ts", "MQ_ALIGNMENT_POSES"),
    };
    return dict;
}
/**********************************************************************
...
function writeOutput(dict) {
    const header = "/**\n" +
        ...
        `export const ALL_PLAYERS_DATA: MQPlayerDictionary = ${JSON.stringify(dict, null, 2)} as const;\n`;
...
function main() {
    ...
    console.log(`📦 Résumé: ${totalBackgrounds} backgrounds, ${totalSkills} skills, ${totalAlignments} alignements (et plus…).`);
